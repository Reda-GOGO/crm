...
import { Building2, Hash, Info, Mail, MapPinned, Phone, PlusSquare, SearchIcon, UserPlus } from "lucide-react";
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader>
        ...
        <CardDescription>
          Select a supplier or create a new one and associate with this purchase order.
        </CardDescription>
      </CardHeader>
  ...
  return (
    ...
    <Tabs defaultValue="create" className="w-full">
      <TabsList className="flex w-full justify-between">
        <TabsTrigger value="create"><UserPlus /> New Supplier</TabsTrigger>
        <TabsTrigger value="search"><SearchIcon /> Find Supplier</TabsTrigger>
      </TabsList>
    ...
    <div>
      <Col>
        ...
        <Field>
          ...
          <div className="relative">
            <MapPinned className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5" />
            <Input
              className="pl-9 h-9 text-sm bg-muted/20"
              placeholder="No 08, Ain Shok Casablanca" />
          </div>
        </Field>
      ...
      <div className="flex w-full justify-center items-center gap-2 pt-6 pb-2">
        <Button variant="default"><PlusSquare /> Add Supplier</Button>
      </div>
    </div>
  )
}
